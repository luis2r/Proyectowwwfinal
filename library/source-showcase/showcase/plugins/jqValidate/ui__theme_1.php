...
<form class="cmxform" id="commentForm" method="post" action="examples/response/commentResponse.php">
  ...
  <fieldset>
    ...
    <p>
      <label for="cname" id="lblCName">Name:</label>
      <input id="cname" name="name" />
      <input id="checkInput" type="checkbox" name="checkInput" />
    <p>
  ...
  ->execute(
    YsUIDialog::build('#dialogId')
     ...
     YsJQValidate::validate('#commentForm')
       ->_rules(
        array('name' => array('required' => "#checkInput:checked", 'minlength' => 2),
...
YsJQuery::click()
  ->in('#checkInput')
  ->handler(
    YsJsFunction::execute(
      YsJQuery::html("Name (required, at least 2 characters):")->in('#lblCName')
        ->condition( // <--> condition(condition,failureCallback)
          YsJQuery::is(':checked')->in("this"), // <- Condition
          YsJQuery::html("Name:")->in('#lblCName') // <- On failure Condition
        )
  )
