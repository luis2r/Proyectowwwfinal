...
$tree->setSelectionLimit(3);

$tree->enableThemeroller();
$tree->enableHotkeysNavigation();
...
<?php echo YsUIDialog::initWidget('dialogId','style="display:none" title="Basic dialog"') ?>
  ...
  <?php echo YsUIButton::initToolbar('toolbarId') ?>
    <button onclick="<?php echo $tree->selectAll() ?>">Select All</button>
    <button onclick="<?php echo $tree->deselectAll() ?>">Deselect All</button>
    <button onclick="<?php echo $tree->selectNode($node00)?>">Select Node 00</button>
    <button onclick="<?php echo $tree->deselectNode($node00)?>">Deselect Node 00</button>
    <button onclick="<?php echo $tree->hoverNode($node0) ?>">Hover Node 0</button>
    <button onclick="<?php echo $tree->dehoverNode($node0) ?>">Dehover Node 0</button>
    <button onclick="<?php echo $tree->toggleSelected() ?>">Toggle Selected</button>
    <button onclick="alert(jQuery('#treeId').jstree('is_selected','node00'))">Is selected Node 00?</button>
    <button onclick="alert(jQuery('#treeId').jstree('get_selected').children('a').text())">Get Selected</button>
