...
<?php echo YsUIDialog::initWidget('dialogId','style="display:none" title="Basic dialog"') ?>
  ...
  <?php echo YsUIButton::initToolbar('toolbarId') ?>
    <button onclick="<?php echo $tree->checkAll()?>">Check All</button>
    <button onclick="<?php echo $tree->uncheckAll()?>">Uncheck All</button>
    <button onclick="<?php echo $tree->checkNode($node00) ?>">Check Node 00</button>
    <button onclick="<?php echo $tree->uncheckNode($node00) ?>">Uncheck Node 00</button>
    <button onclick="alert(jQuery('#treeId').jstree('is_checked','#node00'))">Is checked Node 00?</button>
