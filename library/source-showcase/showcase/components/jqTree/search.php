...
$tree->enableSearch();
$tree->setCaseInsensitive(false);
$tree->bindEvent('search.jstree', 'mySearchFunction');
?>

<script type="text/javascript" language="javascript">
  function mySearchFunction(e, data){
...
<?php echo YsUIDialog::initWidget('dialogId','style="display:none" title="Basic dialog"') ?>
  ...
  <button onclick="<?php echo $tree->search('Node 10') ?>">Search Node 10</button> 
  |
  <button onclick="<?php echo $tree->search(YsJQuery::val()->in('#txtSearchValue')) ?>">Search:</button>
  <input id="txtSearchValue" type="text" />
  |
  <button onclick="<?php echo $tree->clearSearch() ?>">Clear</button>
