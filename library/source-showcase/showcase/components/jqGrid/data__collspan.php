...
$invGridField->setCollspan(2);

$invGridField->setFormatter(YsArgument::value('function(value, options, rData){ return "Invoce: "+value + " - "+rData["name"]; }'));

$dateGridField = new YsGridField('invDate', 'Date');
$dateGridField->displayNone();

$nameGridField = new YsGridField('name', 'Name');

/* The Data (Local Type) */
$grid->addGridFields($invGridField, $nameGridField); // <- multiple
$grid->addGridField($dateGridField); // <- single
