...
YsJQueryAssets::loadScriptsOnReady(array(
  'jquery4php-assets/js/plugins/jqgrid/plugins/grid.setcolumns.js'),
...
$grid->setSortname('client');
  ...
  $navigator->noDefaultButtons();
    ...
    $columnChooser = new YsGridCustomButton();
    $columnChooser->setCaption('Columns');
    $columnChooser->setButtonIcon(YsUIConstant::ICON_NONE);
    $columnChooser->setTitle("Hide/Show Columns");
    $columnChooser->setOnClickButton(new YsJsFunction($grid->invoke('setColumns')));

  $navigator->addCustomButton($columnChooser);
